...
'3_cuda.cu' is the Cuda version of the program. Note that since some Cuda functions such as atomAdd cannot operate on double data, variables are defined as float instead of double, which lead to a slight difference in the outputs compared with the results that we gained before. However, the calculating speed is faster. This program also outputs a file 'cuda_result' to store the data, which can be read and plotted by the python program '4_cuda_plot.py'.

Here are the results on my local machine which has an E3-1230v5 and a GTX 1660S. Also, the generated plots are uploaded into the directory.
